...
public class TelegramBotUpdatesListener implements UpdatesListener {
                        ...
                        case CAT -> {
                            if (userContextService.getByChatId(chatId).isPresent()) {
                                ...
                                if (userContext.getPersonCat() == null) {
                                    ...
                                    personCatService.createPersonCat(personCat);
    ...
    public void getReport(Message message, String petName) {
        ...
        try {
            ...
            reportService.uploadReport(chatId, petName, fileContent, ration,
                    health, behaviour, lastMessage);
            sendForwardMessage(chatId, message.messageId());
            sendResponseMessage(chatId, "Отчет успешно принят!");
        } catch (IOException e) {
    ...
    /**
     * Метод отслеживания отправки отчетов
     */
